...
- **人工版式指令**：在段界与合法段内行位标注「必须保留」（锁定断点）或「禁止断开」（禁断段内位置），
...
| `no_split` | **禁止断开**：该段内行后不得断页 | 段内第 i 行后且 `1 ≤ i ≤ 行数−1` |

- 指令以「段落 id + 段内行号」持久化；草稿修改段落行数、行号越界或段落 id 失配时，
  该指令在响应中**就地判失效**（`invalid_directives`，原因 `paragraph_not_found` /
  `line_out_of_range` / `illegal_position`），不参与约束，其余编辑内容照常求解。
...
- **向后兼容**：请求不带 `directives` 字段时，行为与响应字节级保持原样，
  不出现任何指令相关字段；前端以「段落内部键」关联指令，段落改名不影响引用，
  删除段落时以 id 快照随请求发送，由后端判 `paragraph_not_found` 标出。
...
- 版式指令表：操作（必须保留 / 禁止断开）、目标段落、段内行号；指令以段落内部键关联，
  段落改名跟随、段落删除时以 id 快照保留并就地标出失效（行高亮 + 原因说明），其余编辑不受影响。
